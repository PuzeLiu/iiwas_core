...
class KinematicsTorch:
    def __init__(self, tcp_pos=None, tcp_quat=None):
        self.d_bs = torch.tensor(0.36, dtype=torch.double)
        self.d_se = torch.tensor(0.42, dtype=torch.double)
        self.d_ew = torch.tensor(0.4, dtype=torch.double)
        self.d_wf = torch.tensor(0.151, dtype=torch.double)

        # DH_paramters a_i, alpha_i, d_i
        self.dh_a = torch.tensor([0., 0., 0., 0., 0., 0., 0.], dtype=torch.double)
    ...
    def get_redundancy(self, q):
        ...
        p_0_2 = torch.tensor([0., 0., self.d_bs], dtype=torch.double)
        p_6_7 = torch.tensor([0., 0., self.d_wf], dtype=torch.double)
    ...
    def _get_auxiliary_parameter(self, p, rot_mat, gc):
        ...
        q3_v = torch.tensor(0.0, dtype=torch.double)
        ...
        self.a = torch.zeros((7, 2), dtype=torch.double)
        self.b = torch.zeros((7, 2), dtype=torch.double)
        self.c = torch.zeros((7, 2), dtype=torch.double)
    ...
    def inverse_kinematics(self, pose, psi, gc):
        q = torch.zeros((7,), dtype=torch.double)
